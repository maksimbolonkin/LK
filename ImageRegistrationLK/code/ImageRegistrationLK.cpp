...
Mat ImageRegistrationLK::WindowFunction::getMask()
{
	Mat nim;
			...
			for(int j=0; j<window.height-2; j+=3)
			{
				...
				for(int x = 0; x<3; x++)
					...
					for(int x = 0; x<3; x++)
						for(int y=0; y<3; y++)
						{
							fmap.at<float>(j+y, i+x) = float(mean3x3)<totalMean?0:1;
						}
			}
...
{
	m.copyTo(mask);
	_isMaskSet = 2;		// custom mask
}

void ImageRegistrationLK::setMask(bool mask)
{
	if(mask)
		_isMaskSet = 1;	// default mask
	else
		_isMaskSet = 0;	// no mask	
}
...
Mat ImageRegistrationLK::getMask()
{
	return w.getMask();
}
