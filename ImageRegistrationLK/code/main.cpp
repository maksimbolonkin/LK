...
int main(int argc, char *argv[])
{
	...
	/// Size and offset (top-left corner) of a window to track
	
	//int wx = 120, wy = 110;
	//Point2d offset(50, 20);
	
	// in case all omage needs to be registered
	int wx = fixed.size().width, wy = fixed.size().height;
	Point2d offset(0,0);
	//return 0;
	...
	reg.setRegistrationMode(REG_SCALE|REG_SHIFT);
	reg.setMask(true);	// to set cosine mask (false if no mask)
	//reg.setMask(mask);	// to set custom mask (if known)
	reg.runRegistration();
	...
	imwrite("newimg.jpeg", newimg);

	imwrite(argv[3], abs(fixed-moving));
	imwrite(argv[4], abs(fixed-newimg));
